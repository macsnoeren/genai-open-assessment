...
class Exam {
  ...
  public static function create($title, $description, $docentId) {
    $pdo = Database::connect();
    $stmt = $pdo->prepare("INSERT INTO exams (title, description, docent_id) VALUES (?, ?, ?)");
  ...
  public static function update($id, $title, $description) {
    $pdo = Database::connect();
    $stmt = $pdo->prepare("UPDATE exams SET title = ?, description = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
